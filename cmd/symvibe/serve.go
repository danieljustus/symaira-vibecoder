...
func serveCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var mdns *server.MDNSAdvertiser
			if cfg.Server.Access == "lan" && cfg.Server.MulticastDNS {
				adv, err := server.AdvertiseMulticastDNS(cfg.Server.Host, cfg.Server.Port)
				if err != nil {
					slog.Warn("mDNS advertisement failed", "err", err)
				} else {
					mdns = adv
				}
			}
